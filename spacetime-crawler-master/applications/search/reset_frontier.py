...
from datamodel.search.DelehoybMshoshanUfperez_datamodel import DelehoybMshoshanUfperezLink
...
@Deleter(DelehoybMshoshanUfperezLink)
@Getter(DelehoybMshoshanUfperezLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(DelehoybMshoshanUfperezLink)
        print "Found ", len(ls), " links to delete."
        for l in ls:
            self.frame.delete(DelehoybMshoshanUfperezLink, l)
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_DelehoybMshoshanUfperez".format(CrawlerFrame.app_id),
